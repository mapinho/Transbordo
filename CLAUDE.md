...
One outstanding **manual, production-only** task from Fase 1: before relying on the new `NOT NULL` constraint on `cenario_id` (7 tables), confirm there are zero `cenario_id IS NULL` rows in the real production database, then run the corresponding `ALTER TABLE ... ALTER COLUMN cenario_id SET NOT NULL` migrations (see finding A11). **Update 2026-08-24:** the zero-NULL check was verified against the live legacy `comigo` database during the Fase 5 espelhamento work (`fabricas`, `armazens`, `rotas`, `safras_unidades`, `movimentacoes_diarias` — all zero nulls; see `docs/superpowers/specs/2026-08-24-espelhamento-dados-legado-design.md`). The `ALTER TABLE ... SET NOT NULL` itself has **not** been run — applying it to production is still the project owner's call, not something done as part of that work.
